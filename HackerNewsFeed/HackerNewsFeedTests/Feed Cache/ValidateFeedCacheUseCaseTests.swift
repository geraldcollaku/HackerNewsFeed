...
class ValidateFeedCacheUseCaseTests: XCTestCase {
    ...
    func test_validateCache_doesNotInvalidateCacheAfterSUTInstanceHasBeenDeallocated() {
    ...
    func test_validatedCache_failsOnValidationError() {
        let (sut, store) = makeSUT()
    ...
    func test_validateCache_succeedsOnSuccessfulDeletionOfFailedRetrieveal() {
        let (sut, store) = makeSUT()
    ...
    func test_validateCache_failsWithDeletionErrorOfExpiredCache() {
        let feed = uniqueIdFeed().local
        let fixedCurrentDate = Date()
        let expiredTimestamp = fixedCurrentDate.minusFeedCacheMaxAge().adding(seconds: -1)
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
