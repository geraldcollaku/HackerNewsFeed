...
class CoreDataFeedStoreTests: XCTestCase, FeedStoreSpecs {
    ...
    func test_insert_overridesPreviouslyInsertedCacheValues() async throws {
        try await makeSUT { sut in        assertThatInsertOverridesPreviouslyInsertedCacheValues(on: sut)
        }
    }
    ...
    func test_delete_deliversNoErrorOnNonEmptyCache() async throws {
        try await makeSUT { sut in        assertThatDeleteDeliversNoErrorOnNonEmptyCache(on: sut)
        }
    }
    
    func test_delete_emptiesPreviouslyInsertedCache() async throws {
        try await makeSUT { sut in               assertThatDeleteEmptiesPreviouslyInsertedCache(on: sut)
        }
    }
